...
- [x] **CH01** CHISEL track opening — knowledge cleanup and SPANDEX closure. Model: `claude-opus-4-6`. CI image profile:
  ...
  - spandex-wave-2-baseline: remove the "hidden within-family +3.48 pp nDCG" claim (nDCG-specific; collapses to null
    under AUC). Rewrite as a clean three-null wave. Flag for re-verification under CHISEL frame once CH08 completes.
  ...
  - PROJECT.MD ENTRY: add a single cross-track entry to lyzortx/research_notes/lab_notebooks/project.md documenting (a)
    ...
    concentration, binary) is the training unit, (d) the sequencing of CH02-CH08, and (e) explicit acknowledgement that
...
- [x] **CH06** Panel-independent phage-feature sweep (TL17-bias root-cause attack). Model: `claude-opus-4-6`. CI image
  ...
  - VARIANCE PRE-FLIGHT (required before ARMs 2-4 per lyzortx/AGENTS.md Scientific Review Standards). For each candidate
    ...
    non-lysed pairs on any one of those three subsets, stop and report — do not run the full training for that arm. This
    gate exists because OMP HMM score homogeneity (CV 0.01-0.17) would have been caught before GT02/GT06 burned hours if
    it had been applied then; same pattern could kill any of ARMs 2-4 on BASEL specifically, and we want that caught
    before a 3-hour rerun rather than after.
  - ARM 1 (architectural, calibration arm — NOT discrimination): OOD-aware inference. Compute distance from each
    held-out phage to its nearest training phage (proteome MMseqs2 bit-score or phage_projection Euclidean). Apply
    post-hoc shrinkage toward base rate when distance exceeds a threshold fit on Guelin inner-val. Measurable on
    existing CH05 prediction artifacts without retraining the model — cheapest arm. Shrinkage is monotone so BASEL
    bacteria- axis AUC stays at 0.7152; what moves is BASEL's residual ECE (expected to close most of the 0.11-0.12
    Guelin-fitted-calibrator residual because shrink-toward-base-rate on OOD phages is literally what their
    miscalibration asks for). Report Arm 1 against BASEL ECE, not BASEL AUC.
  - ARM 2: pairwise proteome-similarity vector as replacement for TL17 categorical projection. Compute each phage's
    MMseqs2 bit-score against every training phage. Use the resulting continuous vector (or a PCA/summary of it) as the
    phage-side feature slot. A new phage always produces a full similarity profile; when it is uniformly low that is
    itself a meaningful signal, unlike the TL17 zero-vector failure mode. Strongest structural candidate for closing
    BASEL discrimination AUC. Risk: may cannibalize TL17 (both encode "is this phage similar to known phages"), RFE
    keeps the better one, net ≈ wash. Still the most honest test of categorical-vs-continuous encoding.
  - ARM 3: Moriniere 2026 receptor-class probabilities. Run the Moriniere 19-dim softmax classifier on each phage
    proteome; use the probability vector directly as a phage-side feature (NOT the 815 k-mers — those were tested in
    GT06 and SX12 and were null). Trained on 260 phages spanning non-Guelin panels so it is not Guelin-biased at source.
    **Expected null.** Two knowledge units predict it will fail: (a) same-receptor-uncorrelated-hosts (Tsx phages
    Jaccard 0.091 on host ranges — receptor class is far from sufficient for predicting lysis); (b) Moriniere's
    classifier was trained on BW25113/BL21 which lack capsule/ O-antigen, so polysaccharide-mediated specificity (the
    depo-capsule-validated Gate 1 mechanism, the only validated pairwise signal) is not represented. Probability-output
    vs thresholded-class is a small improvement over the k-mer direct test, not enough to flip the verdict. Keep the arm
    because running it closes the receptor-class thread cleanly — but flag "expected null" so the result does not
    surprise anyone.
  - ARM 4: tail-protein-restricted TL17 BLAST (Shang 2025 benchmark motivation — tail proteins alone match full-genome
    performance). Restrict the TL17 BLAST input to RBP + baseplate + tail-fiber annotated proteins only, re-project.
    **BASEL pre-flight gate** (required before running this arm): count how many of the 52 BASEL phages have
    Pharokka-annotated tail/baseplate/RBP proteins. Restricting the BLAST input to a subset of each phage's proteome can
    only increase the zero-vector fraction. If fewer than 50% of BASEL phages carry those annotations, skip this arm on
    the BASEL side (run Guelin-only) or skip it entirely — the arm cannot rescue a phage whose tail region is not
    annotated.
  - EXPLICIT EXCLUSIONS (known nulls — listed so no future agent re-proposes): SaProt / ProstT5 structural PLM
    (plm-rbp-redundant — -1.22 pp cross-family nDCG, actively harmful); PHROG aggregate category counts
    (phage-functional-noise); dense PHROG categorical membership as a direct TL17 replacement (inherits the same
    zero-vector failure mode — closed vocabulary, just cross-panel instead of Guelin-specific); receptor k-mers as
    direct features (kmer-receptor-expansion-neutral); physicochemical RBP descriptors
    (physicochemical-rbp-insufficient). Panel expansion stays out of CH06 — it is a track-level strategic move per
    panel-size-ceiling, not a feature-experiment arm.
  - ORGANIZING PRINCIPLE: the user's ask is "gradient/numeric proxies that do not assume a closed vocabulary." Arms 1-3
    honor that directly (distance- based, continuous similarity, softmax output). Arm 4 keeps the closed-vocabulary TL17
    projection but shrinks its input scope to the proteins that matter (tail region). Arms that are just new
    closed-vocabulary banks (PHROGs, Pfam) do not honor the organizing principle.
  - ARTIFACTS: ch06_arm{1,2,3,4}_predictions.csv, ch06_arm{1,2,3,4}_metrics.json, ch06_summary.csv (one row per arm:
    bacteria-axis AUC + Brier + CIs, phage-axis AUC + Brier + CIs, per-source breakdowns, delta vs
    chisel-unified-kfold-baseline, BASEL mid-P reliability-gap closure). ch06_arm1_shrinkage_diagnostic.csv (threshold
    sweep for the OOD-aware arm).
  - KNOWLEDGE: if any arm meets the acceptance criterion, add chisel-panel-independent-phage-features unit with the
    ...
    remaining lever."
  - Record per-arm AUC deltas (bacteria-axis and phage-axis, per-source), pre-flight verdicts for each skipped or
    constrained arm, and the verdict on panel-size-ceiling vs feature-level fixes in track_CHISEL.md. The notebook entry
...
- [x] **CH07** Both-axis holdout evaluation (10 x 10 double cross-validation). Model: `claude-opus-4-6`. CI image
  ...
  - GOAL: the strongest generalization test — bacterium AND phage held out simultaneously. Directly addresses the
    ...
    phage-side marginals. Runs against the winning phage-side feature bundle from CH06, not the TL17 baseline that CH05
    diagnosed as broken.
  ...
  - ARTIFACTS: ch07_cell_metrics.csv (per-cell AUC + Brier + observation count), ch07_aggregate.json (pooled AUC + Brier
    with CIs), ch07_cell_distribution.png (histogram of per-cell AUC, to visualize variance).
...
- [x] **CH08** Wave-2 feature-family re-audit under CHISEL frame. Model: `claude-opus-4-6`. CI image profile: `base`.
  ...
  - ARTIFACTS: ch08_sx12_delta.json, ch08_sx13_delta.json, ch08_summary.csv (one table: baseline vs +moriniere vs
