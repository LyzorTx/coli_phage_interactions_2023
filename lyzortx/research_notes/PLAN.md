...
- [ ] **CH06** Panel-independent phage-feature sweep (TL17-bias root-cause attack). Model: `claude-opus-4-6`. CI image
  ...
  - ARM 1 (architectural, no retraining): OOD-aware inference. Compute distance from each held-out phage to its nearest
    training phage (proteome MMseqs2 bit-score or phage_projection Euclidean). Apply post-hoc shrinkage toward base rate
    when distance exceeds a threshold fit on Guelin inner-val. Measurable on existing CH05 prediction artifacts without
    retraining the model — cheapest arm.
  - ARM 2: pairwise proteome-similarity vector as replacement for TL17 categorical projection. Compute each phage's
    ...
    itself a meaningful signal, unlike the TL17 zero-vector failure mode.
  - ARM 3: Moriniere 2026 receptor-class probabilities. Run the Moriniere 19-dim softmax classifier on each phage
    ...
    Upside capped by same-receptor-uncorrelated-hosts (same-receptor phages can have uncorrelated host ranges, Jaccard
    0.091 for Tsx), so expect small lift at best — include anyway for completeness.
  - ARM 4: tail-protein-restricted TL17 BLAST (Shang 2025 benchmark motivation — tail proteins alone match full-genome
    ...
    Same feature structure as current phage_projection, cleaner input.
