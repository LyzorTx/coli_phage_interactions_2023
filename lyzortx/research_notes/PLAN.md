...
- [ ] **SX06** Continuous depolymerase features (generalization-friendly). Model: `claude-opus-4-6`. CI image profile:
  ...
  - MOTIVATION: Current depo×capsule cross-terms use binary cluster membership (in_cluster_N × capsule_score) built from
    ...
    sequences — they cannot generalize to unseen depolymerases. Same problem affects k-fold CV within our panel. Scope
    discipline: keep LightGBM for this track; set-aware architectures (two-tower, Set Transformer) are noted as a future
    direction in project.md — do NOT pursue them here.
  ...
  - MINIMAL MEAN-POOLING APPROACH: Compute ESM-2 embeddings per depolymerase protein (not mean-pooled across all RBPs).
    Within each protein, prefer max-pooling or regional pooling (N-terminal + C-terminal halves separately) over
    mean-pooling — max-pooling preserves the strongest motif-level signal. Only mean-pool across proteins of the same
    functional class when a phage has multiple depolymerases. Keep phage_depo_plm_* features separate from the existing
    phage_rbp_struct block (which stays as aggregated mean-pool for comparison). PCA each new feature block separately
    to 16-32 dims.
  - Evaluate both approaches (MMseqs2 bitscore and per-depo PLM) via k-fold CV (within-panel generalization) and SX05
    Arm C (unseen-phage generalization). Compare nDCG, mAP, AUC against SX05 baseline.
  - If continuous features improve generalization to unseen phages by >2pp AUC, adopt as default feature representation
    for Gate 1 cross-terms
