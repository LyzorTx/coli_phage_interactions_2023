...
- [ ] **SX09** MLC=1 exclusion sensitivity analysis. Model: `claude-opus-4-6`. CI image profile: `base`. Depends on
      tasks: `SX01`.
  - MOTIVATION: Paper flags lawn clearing at high phage concentration as potentially non-productive (lysis from without
    or abortive infection). Maps to MLC=1 in our scoring — lysis only at MOI~10, zero at lower dilutions (no evidence of
    productive amplification). MLC=1 is 7.8% of all pairs and 46% of positives. Training on MLC=1 pairs may be training
    on non-productive lysis signal that is clinically irrelevant and potentially misleading.
  ...
  - Build three training cohorts from clean-label data (already excluding n-scores from GT09): (a) all positives (SX01
    baseline); (b) exclude MLC=1 positives (treat as unobserved, not negative); (c) keep MLC=1 but downweight to 0.5x in
    training
  - Evaluate all three on SPANDEX metric suite with 10-fold CV
